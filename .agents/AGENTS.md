...
| `HybridGeneticOptimizer.ts` | The Genetic Algorithm engine. **Population: 100 chromosomes, 40 generations** (with stagnation limit of 15 gens). Uses tournament selection (size 2), single-point crossover, 15% mutation rate, 10% elitism. Fitness = multi-objective Calmar Ratio × Trade Count Significance × Win Rate Floor × Monthly Consistency. Early exit on convergence. Local exhaustive search refines top 3 solutions post-GA. |
| `WalkForwardEngine.ts` | Generates Walk-Forward Analysis (WFA) windows. **6-month In-Sample, 2-month Out-of-Sample, stepping forward by 2 months.** Partial final OOS windows with < 14 days of data are dropped. This 2-month step size is the fundamental unit that determines optimizer re-run frequency. |
