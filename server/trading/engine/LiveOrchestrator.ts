  ...
  async start() {
    ...
    try {
      ...
      for (const t of openTrades) {
        ...
        for (const sp of matchingPairs) {
          ...
          if (!state.activeTrades.some((at: any) => at.metaOrderId === t.meta_order_id)) {
    ...
    try {
      ...
      for (const pos of allPositions) {
        ...
        // Skip if not a symbol we manage AND not explicitly tagged by our bots
        if (!managedSymbols.has(posBaseSymbol) && !managedSymbols.has(pos.symbol) && !isKnownBotTrade) {
          continue;
        }
        ...
        try {
          ...
          for (const sp of matchingPairs) {
            ...
            if (!state.activeTrades.find((t) => t.metaOrderId === pos.id)) {
              state.activeTrades.push({
                dbId,
                metaOrderId: pos.id,
        ...
        Array.from(this.states.entries())
          ...
          .map(([p, s]) => [
            p,
            {
              ...
              hasActiveTrade: s.activeTrades && s.activeTrades.length > 0,
              activeTradesList: s.activeTrades || [],
              hasLimitOrder:
  ...
  handlePositionUpdate(pos) {
    ...
    for (const sp of matchingPairs) {
      ...
      if (state && state.activeTrades) {
        const trade = state.activeTrades.find(t => t.metaOrderId === pos.id);
        if (trade) {
