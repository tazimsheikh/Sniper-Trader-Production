...
if (!isMainThread) {
  ...
  for (let sim = 0; sim < simulations; sim++) {
    ...
    if (category.includes("Highest Net Profit")) fitness = totalReturn;
      ...
      fitness =
        rawMaxDrawdown === 0
          ? 99999  // Perfect zero drawdown = maximum fitness score
  ...
  async function runPipeline() {
    ...
    console.log("Found files:", stateFiles.length);
    for (const file of stateFiles) {
      ...
      const purgedData = data.filter((config) => {
        ...
        if (metrics.profitFactor < 1.15) return false;
        if (metrics.winRate < 30.0) return false;
      ...
      for (const config of purgedData) {
        ...
        const maxBodyPart = parts.find((p: string) => p.startsWith("MaxBody"));
        const bypassPart = parts.find((p: string) => p.endsWith("%"));
        const signature = `${session}_${bypassPart}_${maxBodyPart}_${fcPart}`;
    ...
    const outPath = path.join(
      ...
      "trading",
      "optimizer",
      "sage_monte_carlo_portfolios.md",
    );
