  ...
  const runWorker = async () => {
    ...
    for (const session of sessions) {
      for (const window of windows) {
        ...
        const getCachedTriggers = (minBody: number, wickRatio: number, candles: any[], ema: any, bb: any[], rsi: any[]) => {
          const key = `${minBody}_${wickRatio}`;
        ...
        const numericIndices = Array(8).fill(0).map((_, idx) => idx);
        
        // Use an IIFE for await since we're in a synchronous block loop otherwise.
        const bestISPromise = optimizer.optimize(numericIndices);
        bestISPromise.then(bestIS => {
          ...
          for (const candidate of top3) {
            ...
            if (topIS.totalNetR > 0 && topIS.trades >= 5) {
              const oosTriggers = getCachedTriggers(minBody, wickRatio, oosM5, oosEma, oosBb, oosRsi);
        ...
        // Actually we are inside an async block loop already! Let's await it directly.
      }
    }
  };
