...
function cleanOldDumps() {
  console.log(`\n[1/6] 🧹 Archiving previous dump files to last_optimization_run (if older than 20 days)...`);
  let moved = 0;
  const now = Date.now();
...
async function fetchHistoricalData() {
  console.log(`\n[2/6] 📈 Hydrating CSVs from MetaApi...`);
...
async function runMasterSynthesizer() {
  console.log(
    `\n================================================================`,
  );
  console.log(`👑 GRANDMASTER SYNTHESIZER ORCHESTRATOR`);
  console.log(`   (Fast Pipeline: Hydration → DNA Banks → Synthesis → Injection)`);
  console.log(
    `================================================================`,
  );
  ...
  try {
    ...
    console.log(`\n🎉 MASTER SYNTHESIZER COMPLETED SUCCESSFULLY!`);
    console.log(`🏆 Holy Grail Portfolio injected to PairConfig.ts (Mage + Sage + Seer)`);
    process.exit(0);

  } catch (err: any) {
    console.error(`\n🔥 FATAL SYNTHESIZER ERROR: ${err.message}`);
    process.exit(1);
  }
}
