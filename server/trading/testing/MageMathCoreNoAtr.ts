  ...
  for (let i = 2; i < m5Candles.length - 1; i++) {
    ...
    if (
      ...
      if (sellTriggered && Math.min(actionCandle.open, actionCandle.close) > orLow) continue;
        ...
        if (mCfg && mCfg.maxOpposingWickRatio !== undefined) {
          ...
          const wbr = opposingWickPips / Math.max(cBodyPips, 0.1);
          if (wbr > mCfg.maxOpposingWickRatio) continue;
        }

        if (mCfg && mCfg.htfAlignmentRequired) {
          const direction = buyTriggered ? "BUY" : "SELL";
          if (HTFContextTracker.isTrendParabolicFast(htfData, i, direction, mCfg.maxH1EmaSlope ?? 30, pipSize)) {
            continue;
          }
        }
        ...
        if (atrVal > 0) {
          ...
          // DISABLED FOR TEST: if (ratio < minRatio || ratio > maxRatio) continue;
        }

        if (mCfg && mCfg.maxSlDist !== undefined) {
          const maxSlPips = mCfg.maxSlDist;
          if (orPips + (spreadPts / pipSize) > maxSlPips) continue;
        }
...
export function evaluateExits(
  ...
  for (const t of triggers) {
    ...
    // IMPORTANT: Start searching from t.m1Index + 5 (the close of the M5 action candle)
    // because the trigger is generated at the CLOSE of the M5 candle. Price action
    // during the action candle itself cannot fill the limit order or hit SL/TP.
    const searchStartIndex = Math.min(m1.length - 1, t.m1Index + 5);
    for (
      let j = searchStartIndex;
      j < Math.min(m1.length, searchStartIndex + 5000);
      j++
    ) {
      ...
      if (isEOD) {
        ...
        if (!tradeActive && !missedTrade) {
          ...
          if (!config.orbPullbackPct || config.orbPullbackPct === 0 || isInstantFill) {
            entryTimeMs = m1.timestamp[j];
            const filledPrice = direction === "BUY" ? m1.open[j] + spreadPts : m1.open[j];
            if ((direction === "BUY" && filledPrice <= slPrice) || (direction === "SELL" && filledPrice >= slPrice)) {
              tradeActive = true;
              outcome = "SL";
              rMultiple = -1.0;
              break;
              ...
              initialRisk = Math.abs(entryPrice - slPrice);
            }
          } else if (direction === "BUY" && m1.low[j] + spreadPts <= limitBuyPrice) {
            entryTimeMs = m1.timestamp[j];
            const filledPrice = Math.min(m1.open[j] + spreadPts, limitBuyPrice);
            if (filledPrice <= slPrice) {
              tradeActive = true;
              outcome = "SL";
              rMultiple = -1.0;
              break;
          ...
          } else if (direction === "SELL" && m1.high[j] >= limitSellPrice) {
            entryTimeMs = m1.timestamp[j];
    ...
    if (outcome !== "SKIPPED") {
      ...
      const exitTimeMs = m5Candles[t.m5Index]?.timestamp ?? 0;
