...
async function loadPortfolioTrades(startDate: string, endDate: string): Promise<any[]> {
  ...
  if (fs.existsSync(jsonPath)) {
    ...
    for (const comp of portfolio) {
      ...
      const cfg = liveCfgs && liveCfgs.length > 0 ? { ...liveCfgs[0] } : parseSetupToConfig(comp.setup, comp.symbol, isSage);
      try {
        const res = isSage
...
async function runCohortsModule(trades: any[]) {
  console.log("\n====================================================================================================");
  console.log(" 🔬 MODULE 3: 1-YEAR ROLLING INCEPTION COHORT SURVIVAL AUDIT");
  console.log("====================================================================================================");
  ...
  for (const startStr of cohortMonths) {
    const startMs = new Date(startStr).getTime();
    const cohortTrades = trades.filter(t => new Date(t.exitTime || t.entryTime || t.date || t.time).getTime() >= startMs);
    ...
    results.push({
      cohortStart: startStr.substring(0, 7),
      tradesCount: cohortTrades.length,
...
async function runInstitutionalGridModule(trades: any[]) {
  ...
  function evaluateInstitutionalScenario(baseRisk: number, dailyCapPct: number, peakToDrawPct: number) {
    ...
    for (const t of trades) {
      ...
      const dateStr = new Date(t.date || t.exitTime || t.entryTime).toLocaleString("en-US", { timeZone: "America/New_York" });
  ...
  // 1. Account A (Conservative Profile — 2.5% Daily Cap / 6.0% Trailing Max DD)
...
async function main() {
  ...
  console.log(` 🕹️  Command Mode: "${arg}"`);
  console.log(`========================================================================`);

  const fullTrades = await loadPortfolioTrades("2023-08-01", "2026-08-14");
  const ytdTrades = fullTrades.filter(t => {
    const d = t.exitTime || t.entryTime || t.date || t.time || "";
    return d >= "2026-02-01" && d <= "2026-07-31T23:59:59Z";
  });

  console.log(`📦 Loaded ${fullTrades.length} Multi-Year Trades (Aug 2023 – Aug 2026)`);
  console.log(`📦 Loaded ${ytdTrades.length} YTD 2026 Out-Of-Sample Trades (Feb 2026 – Jul 2026)\n`);

  if (arg === "modes" || arg === "--modes") {
    await runRiskModesModule(ytdTrades);
  } else if (arg === "monthly" || arg === "--monthly") {
    await runMonthlyModule(fullTrades);
  } else if (arg === "cohorts" || arg === "--cohorts") {
    await runCohortsModule(fullTrades);
  } else if (arg === "institutional" || arg === "--institutional" || arg === "grid" || arg === "--grid") {
    await runInstitutionalGridModule(ytdTrades);
  } else {
    // Run all modules
    await runRiskModesModule(ytdTrades);
    await runMonthlyModule(fullTrades);
