...
async function runSinglePair(pair: string, minRatio: number): Promise<RunResult> {
  ...
  const triggers = preComputeTriggersWithRatio(
    m5Candles, m1Rows, atrArr, pair, configs, isForex, spreadPts, pipSize, minRatio, 1.5
  );
...
async function main() {
  ...
  for (const ratio of minRatios) {
    ...
    console.log(`ATR Min Ratio: ${ratio.toFixed(2)} | Total Net R: ${data.totalNetR.toFixed(2)} | Total Trades: ${data.totalTrades}`);
  }

  console.log(`\n--- Best Performers ---`);
  let bestRatio = 0.35;
  let bestR = resultsByRatio[0.35].totalNetR;
  for (const ratio of minRatios) {
  ...
  console.log(`BEST RATIO: ${bestRatio.toFixed(2)} with ${bestR.toFixed(2)}R`);

  let md = `# MAGE ATR Filter Min Ratio Optimization (Last 6 Months)\n\n`;
  md += `| Min Ratio | Total Net R | Total Trades |\n`;
  md += `|-----------|-------------|--------------|\n`;
  for (const ratio of minRatios) {
    const data = resultsByRatio[ratio];
    md += `| **${ratio.toFixed(2)}**${ratio === bestRatio ? ' 🏆' : ''} | ${data.totalNetR.toFixed(2)}R | ${data.totalTrades} |\n`;
  }
  md += `\n## Pair Breakdown\n\n`;
  
  md += `| Pair |`;
  for (const ratio of minRatios) md += ` ${ratio.toFixed(2)} Net R |`;
  md += `\n|---|`;
  for (const ratio of minRatios) md += `---|`;
  md += `\n`;
  ...
  fs.writeFileSync("C:/Users/tazim/.gemini/antigravity/brain/b2f5fb21-b306-4513-8cf6-5dacb6d1c255/atr_min_ratio_results.md", md);
  console.log("\nResults written to atr_min_ratio_results.md artifact.");
}
