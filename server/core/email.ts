...
export async function sendOtpEmail(
  ...
  if (!host || !user || !pass) {
    console.warn(
      "[Email Service] ⚠️ SMTP environment variables are not fully configured.",
    );
    console.warn(
      "[Email Service] ⚠️ To enable real email delivery, set SMTP_HOST, SMTP_USER, SMTP_PASS in your environment.",
    );
    console.warn(
      "[Email Service] 👉 Fallback activated: Access code was successfully logged above.",
    );
  ...
  try {
    ...
    console.error(
      `[Email Service] ❌ Failed to send verification email to ${email}:`,
      err.message,
    );
    return false;
  }
