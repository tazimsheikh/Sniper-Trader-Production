...
export class AUDJPYNyFadeBot extends TradingBot {
  config: BotConfig = {
    ...
    description: 'Fades the New York session drift on AUDJPY. Executes at 15:30 UTC (-10m latency offset). Uses 240 EMA trend fade with 50 pip TP / 20 pip SL.',
    symbols: ['AUDJPY'],
    riskPct: 5,
  ...
  async generateSignal(context: BotContext): Promise<BotSignal> {
    ...
    if (brokerSymbol !== 'AUDJPY') return { shouldTrade: false };
    ...
    if (now.getUTCHours() === 15 && now.getUTCMinutes() === 20) {
      ...
      console.log(`[AUDJPY-Fade] Triggered at ${currentPrice}. EMA is ${this.htfEma.toFixed(3)}. Fading: ${direction}.`);
