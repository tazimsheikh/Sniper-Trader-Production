...
export class GBPAUDNyFadeBot extends TradingBot {
  config: BotConfig = {
    ...
    description: 'Fades the New York session drift on GBPAUD. Executes at 14:30 UTC (-10m latency offset). Uses 240 EMA trend fade with 50 pip TP / 20 pip SL.',
    symbols: ['GBPAUD'],
    riskPct: 5,
    strategyType: 'REVERSAL',
    color: 'slate',
    icon: '⚡',
    winRateBacktest: 55.4,
    returnBacktest: '+14.4M% / 5yr',
    maxDDBacktest: 25.4,
  };
  ...
  async generateSignal(context: BotContext): Promise<BotSignal> {
    ...
    if (brokerSymbol !== 'GBPAUD') return { shouldTrade: false };
    ...
    if (now.getUTCHours() === 14 && now.getUTCMinutes() === 20) {
      const direction = currentPrice > this.htfEma ? 'SELL' : 'BUY';
      console.log(`[GBPAUD-Fade] Triggered at ${currentPrice}. EMA is ${this.htfEma.toFixed(3)}. Fading: ${direction}.`);
