...
export class GBPCHFLondonFadeBot extends TradingBot {
  config: BotConfig = {
    ...
    description: 'Fades the London session drift on GBPCHF. Executes at 9:00 UTC (-10m latency offset). Uses 240 EMA trend fade with 50 pip TP / 20 pip SL.',
    symbols: ['GBPCHF'],
    riskPct: 5,
    strategyType: 'REVERSAL',
    color: 'slate',
    icon: '⚡',
    winRateBacktest: 64.8,
    returnBacktest: '+17.6M% / 5yr',
    maxDDBacktest: 22.8,
  };
  ...
  async generateSignal(context: BotContext): Promise<BotSignal> {
    ...
    if (brokerSymbol !== 'GBPCHF') return { shouldTrade: false };
    ...
    if (now.getUTCHours() === 8 && now.getUTCMinutes() === 50) {
      ...
      console.log(`[GBPCHF-Fade] Triggered at ${currentPrice}. EMA is ${this.htfEma.toFixed(3)}. Fading: ${direction}.`);
