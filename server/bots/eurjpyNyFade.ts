...
export class EURJPYNyFadeBot extends TradingBot {
  config: BotConfig = {
    id: 'eurjpy-ny-fade',
    name: 'EURJPY Ny Fade',
    tagline: 'High-Probability Fade',
    description: 'Fades the New York session drift on EURJPY. Executes at 15:30 UTC (-10m latency offset). Uses 240 EMA trend fade with 50 pip TP / 20 pip SL.',
    symbols: ['EURJPY'],
    riskPct: 5,
  ...
  async generateSignal(context: BotContext): Promise<BotSignal> {
    ...
    if (brokerSymbol !== 'EURJPY') return { shouldTrade: false };
    ...
    if (now.getUTCHours() === 15 && now.getUTCMinutes() === 20) {
      ...
      console.log(`[EURJPY-Fade] Triggered at ${currentPrice}. EMA is ${this.htfEma.toFixed(3)}. Fading: ${direction}.`);
...
export default new EURJPYNyFadeBot();
