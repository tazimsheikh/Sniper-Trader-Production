...
export class GbpusdLondonFadeBot extends TradingBot {
  config: BotConfig = {
    ...
    description: 'Fades the London session drift on GBPUSD precisely at 07:15 UTC. It uses the 4H trend (240 EMA) and fades it with a 100 pip TP and 30 pip SL, achieving +30.6M% return over 5 years.',
    symbols: ['GBPUSD'],
    riskPct: 5,
    strategyType: 'REVERSAL',
    color: 'indigo',
    icon: '⚡',
    winRateBacktest: 71.9,
    returnBacktest: '+30.6M% / 5yr',
    maxDDBacktest: 17.5,
  };
  ...
  async generateSignal(context: BotContext): Promise<BotSignal> {
    ...
    if (now.getUTCHours() === 7 && now.getUTCMinutes() === 5) {
      ...
      console.log(`[GBPUSD-LondonFade] Triggered at ${currentPrice}. 4H EMA is ${this.htfEma.toFixed(5)}. Fading: ${direction}.`);
