...
import { SAGE_PAIR_CONFIG } from './server/trading/config/PairConfig.js';
...
async function testSageParity() {
    ...
    const pairs = Object.keys(SAGE_PAIR_CONFIG || {});
    for (const pair of pairs) {
        console.log('Testing SAGE ' + pair + '...');
        const mathStats = await runSageMathBacktest(pair, startDate, endDate, true, undefined, undefined, null, undefined, false);
