...
import { buildEmaArray } from "../server/trading/market/Indicators.js";
import { DailyContextTracker } from "../server/trading/market/DailyContextTracker.js";
import {
  MAGE_PAIR_CONFIG,
  SAGE_PAIR_CONFIG,
  PairConfigManager,
} from "../server/trading/config/PairConfig.js";
import { OPTIMIZER_CONFIG } from "../server/trading/config/OptimizerPairConfig.js";
import { TradeRecord, TradeOutcome, M1TypedArrays } from "../server/trading/config/types.js";
...
import { preComputeTriggers, evaluateExits } from "./IsolatedSageMathCore.js";
...
export async function runSageMathBacktest(
  ...
  if (backtestCache.has(cacheKey)) {
    ...
    atrArr = global.buildAtrArray(m5Candles, 14);

    backtestCache.set(cacheKey, { m5Candles, emaArr, atrArr, m1Rows });
  }
  ...
  for (const loopConfig of sageConfigs) {
    ...
    const evalRes = evaluateExits(
      ...
      triggers,
      pair,
      { ...config, _atrArr: atrArr, _emaArr: emaArr } as any,
      sessionName,
      isForex,
      0
    );
