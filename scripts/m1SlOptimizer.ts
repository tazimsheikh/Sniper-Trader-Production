...
function runM1Simulation(globalM1Candles: Candle[], isGold: boolean, isJpy: boolean, pullbackTarget: number, slBufferPips: number) {
...
async function runAnalysis() {
  ...
  const targetPairs = [
    { sym: 'GBPUSD', file: 'GBPUSD_M1_202105030000_202605010159.csv', pb: 8 },
    { sym: 'EURCAD', file: 'EURCAD_M1_202105030000_202605010159.csv', pb: 15 },
    { sym: 'XAUUSD', file: 'XAUUSD_M1_202105030000_202605010159.csv', pb: 15 }
  ];
  
  const slBuffers = [2, 5, 10, 15]; // Pips to buffer the stop loss

  for (const pair of targetPairs) {
    console.log(`\nAnalyzing ${pair.sym} (Optimal Pullback: ${pair.pb} pips)`);
    const data = await loadData(path.join(dataDir, pair.file));
    for (const buf of slBuffers) {
       const isGold = pair.sym === 'XAUUSD';
       const res = runM1Simulation(data, isGold, false, pair.pb, buf);
       console.log(`  SL Buffer: ${buf} pips -> WR: ${res.wr.toFixed(1)}% | Return: +${res.ret.toFixed(1)}%`);
    }
    data.length = 0;
  }
}
