...
function runSimulation(globalM1Candles: Candle[], config: OptimizerConfig, isGold: boolean, isJpy: boolean) {
  ...
  for (let cIdx = 0; cIdx < globalM1Candles.length; cIdx++) {
    ...
    for (const trade of openTrades) {
      ...
      if (config.usePyramid && !trade.pyramidAdded && trade.highestProfitPips >= Math.abs(trade.entryPrice - trade.slPrice)/PIP_SIZE) {
         trade.pyramidAdded = true;
         trade.slPrice = trade.entryPrice; // Move SL to Breakeven
         trade.lotsRemaining *= 2; // Double the position
      }

      // Runner Partial Take Profit Logic
      if (config.useRunner && !trade.partialTaken && currentPips >= (Math.abs(trade.entryPrice - trade.slPrice)/PIP_SIZE) * config.baseRr) {
         trade.partialTaken = true;
         const profitToBook = (currentPips * PIP_VALUE) * (trade.lotsRemaining * 0.8);
         trade.totalProfit += profitToBook;
         BALANCE += profitToBook;
         peakBalance = Math.max(peakBalance, BALANCE);
         trade.lotsRemaining *= 0.2; // Leave 20% running
      }

      // Runner Trailing Stop Logic (Trail by 20 pips once partial is taken)
      if (config.useRunner && trade.partialTaken) {
         if (trade.direction === 'BUY') {
            const newSl = c.high - (20 * PIP_SIZE);
            if (newSl > trade.slPrice) trade.slPrice = newSl;
         } else {
            const newSl = c.low + (20 * PIP_SIZE);
            if (newSl < trade.slPrice) trade.slPrice = newSl;
         }
      }
      ...
      if (trade.direction === 'BUY') {
        if (c.low <= trade.slPrice) {
          ...
          trade.closePips = lossPips; closedTrades.push(trade);
        } else if (!config.useRunner && c.high >= trade.tpPrice) {
        ...
        if (c.high + spreadVal >= trade.slPrice) {
          ...
          peakBalance = Math.max(peakBalance, BALANCE); maxDD = Math.max(maxDD, ((peakBalance-BALANCE)/peakBalance)*100); 
          trade.closePips = lossPips; closedTrades.push(trade);
        } else if (!config.useRunner && c.low <= trade.tpPrice) {
    ...
    if (nyMinute % 15 === 14 && m15Candles.length >= 2) {
       ...
       if (pipsAboveAsian >= 10) {
          if (triggerCandle.close < triggerCandle.open) { // Bearish Engulf
             if (bodySize / candleRange > 0.4) direction = 'SELL';
          }
       } 
       // Low Trap (Buy)
       else if (pipsBelowAsian >= 10) {
          if (triggerCandle.close > triggerCandle.open) { // Bullish Engulf
       ...
       if (direction) {
          ...
          const tpPips = slPips * config.baseRr;
          ...
          openTrades.push({
            ...
            pyramidAdded: false, partialTaken: false, totalProfit: 0
          });
          hasTradedToday = true;
       }
    }
  ...
  return { trades: wins+losses, wr: winRate*100, ret: totalReturn, dd: maxDD };
}
...
async function runAll() {
  ...
  const configs: OptimizerConfig[] = [
    { name: 'Baseline (Flat 1:3 RR)', useRunner: false, usePyramid: false, baseRr: 3.0 },
    { name: 'The Runner (80% at 1:3, Trail 20%)', useRunner: true, usePyramid: false, baseRr: 3.0 },
    { name: 'Pyramiding (Double down at 1R)', useRunner: false, usePyramid: true, baseRr: 3.0 }
  ];

  console.log(`Starting massive 17-pair backtest...`);
  ...
  for (const config of configs) {
    ...
    for (const file of files) {
      ...
      avgWinRate += res.wr;
      portfolioReturn += res.ret; // Simple additive combination for the portfolio
      pairsProcessed++;
