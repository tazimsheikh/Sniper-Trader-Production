...
function analyzeForensics(m1: Candle[], pairName: string, pipSize: number, rrTarget: number) {
  ...
  for (let i = 0; i < m1.length; i++) {
    ...
    if (h >= 7 && h < 14) { 
      ...
      if (asianH > -Infinity && boxPips <= 30) {
        if (trapState === 'NONE') {
          const bullTrapDist = (huntH - asianH) / pipSize;
          const bearTrapDist = (asianL - huntL) / pipSize;
          if (macroTrend === 'SHORT' && bullTrapDist >= 5 && bullTrapDist <= 20) trapState = 'BULL_TRAPPED';
          else if (macroTrend === 'LONG' && bearTrapDist >= 5 && bearTrapDist <= 20) trapState = 'BEAR_TRAPPED';
        }
      }
    }
    ...
    // HARD FILTERS (Mon/Tue/Thu, Block 7/8/14)
...
async function main() {
  ...
  console.log('GENERATING DEEP DIVE METRICS CSV (1:5 RR, 5 YEARS, MON/TUE/THU)');
  console.log('═'.repeat(100));
  ...
  const csvRows = ['Pair,Date,Status,DayOfWeek,EntryHour,AsianBoxPips,TrapDepthPips,EngulfingPips,EngulfRatio,SLPips,ADR14Pips,TimeInTrapMins,DistToPDH,DistToPDL,MinutesInTrade'];
  
  for (const t of allTrades) {
    csvRows.push(`${t.pair},${t.date},${t.status},${t.dayOfWeek},${t.entryHour},${t.asianBoxPips.toFixed(1)},${t.trapDepthPips.toFixed(1)},${t.engulfingPips.toFixed(1)},${t.engulfRatio.toFixed(2)},${t.slPips.toFixed(1)},${t.adr14Pips.toFixed(1)},${t.timeInTrapMins.toFixed(0)},${t.distToPDH.toFixed(1)},${t.distToPDL.toFixed(1)},${t.minutesInTrade.toFixed(0)}`);
  }

  fs.writeFileSync(path.join(process.cwd(), 'deep_dive_metrics.csv'), csvRows.join('\\n'));
  console.log(`\\nExported ${allTrades.length} trades to deep_dive_metrics.csv`);
}
