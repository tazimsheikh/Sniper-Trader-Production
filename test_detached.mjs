import pg from 'pg';
const { Client } = pg;
const client = new Client({ connectionString: 'postgresql://postgres.wfumcesezszoyyquqryy:TAZIMsheikh1%23@aws-1-ap-northeast-1.pooler.supabase.com:6543/postgres?pgbouncer=true' });
await client.connect();
const trades = await client.query("SELECT id, profile_id, bot_id, broker_symbol, meta_order_id, manages_own_trailing FROM bot_trade_states WHERE manages_own_trailing = 0 ORDER BY open_time DESC LIMIT 20");
console.table(trades.rows);
await client.end();
