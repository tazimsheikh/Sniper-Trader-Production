...
console.log('Fetching specification for GER40 from MetaAPI...');
try {
...
console.log('Fetching all symbols containing GER, DAX, DE...');
try {
  const res2 = await fetch('https://mt-client-api-v1.agiliumtrade.ai/users/current/accounts/' + rawAcc + '/symbols', { headers: { 'auth-token': rawToken } });
  const data2 = await res2.json();
  if (Array.isArray(data2)) {
    const names = data2.map(s => s.symbol);
